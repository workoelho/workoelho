...
export const url = "/organizations/:organizationId(\\d+)/providers/new";
...
function Page({ session }: Props) {
  ...
  return (
    <Layout title="New provider" session={session}>
      <h1>Create new provider</h1>

      <form
        method="POST"
        action={`/organizations/${session.user.organizationId}/providers`}
      >
        <div>
